...
const props = defineProps({
    ...
    problemType: {
        type: String,
        required: true,
        validator: (value) => ['problem_set', 'python_basics'].includes(value)
    },
    itemLabel: {
        type: String,
        default: 'Problem'
...
const fetchProblems = async () => {
    try {
        const response = await fetch(`http://localhost:8000/problems/list/?type=${props.problemType}`, {
            method: 'GET',
        ...
        if (props.sortByOrder) {
            data = data.sort((a, b) => a.order - b.order);
        }
        problems.value = data;
    } catch (err) {
        error.value = `Failed to fetch ${props.itemLabel.toLowerCase()}s: ` + err;
    } finally {
        loading.value = false;
    }
};
...
<template>
    <div class="p-6">
        <h2 class="text-2xl font-semibold text-center mb-4 text-surface-900 dark:text-surface-0">{{ title }}</h2>

        <div v-if="loading" class="text-center text-gray-500">Loading {{ itemLabel.toLowerCase() }}s...</div>
        ...
        >
            <template #header>
                <div class="flex justify-between items-center">
                    ...
                    <InputText v-model="filters.global.value" :placeholder="`Search ${itemLabel}s...`" class="p-inputtext-sm w-60" />
                </div>
            </template>

            <Column field="name" :header="`${itemLabel} Name`" style="min-width: 14rem" :sortable="true">
                <template #body="{ data }">
            ...
            <Column v-if="problemType === 'problem_set'" field="language" header="Language" style="min-width: 10rem" :sortable="true">
                <template #filter="{ filterModel }">
            ...
            <template #empty> No {{ itemLabel.toLowerCase() }}s found. </template>
            <template #loading> Loading {{ itemLabel.toLowerCase() }}s data. Please wait... </template>
        </DataTable>

        <div v-if="!loading && !problems.length" class="text-center text-gray-500">No {{ itemLabel.toLowerCase() }}s available.</div>
    </div>
</template>
